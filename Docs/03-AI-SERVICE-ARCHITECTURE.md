...
class HybridRecommender:
    def __init__(self, model_path: str):
        self.collaborative_model = tf.keras.models.load_model(
            f"{model_path}/collaborative_model.h5"
        )
        self.content_based_model = tf.keras.models.load_model(
            f"{model_path}/content_based_model.h5"
        )
...
class ModelTrainer:
    ...
    def train(self, train_data, val_data, epochs: int, batch_size: int):
        # MLflow tracking
        mlflow.start_run()
        mlflow.log_params({
